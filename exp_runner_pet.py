...
from models.fields_pet import RenderingNetwork, SDFNetwork, SingleVarianceNetwork, NeRF
...
class Runner:
    def __init__(self, conf_path, mode='train', case='CASE_NAME', is_continue=False, ckpt_name=None, base_exp_dir=None, end_iter=None):
        ...
        self.sdf_network = TriPlaneGenerator(**self.conf['model.triplane_network']).to(self.device)
        self.deviation_network = SingleVarianceNetwork(**self.conf['model.variance_network']).to(self.device)
        self.color_network = RenderingNetwork(**self.conf['model.rendering_network']).to(self.device) # None #
    ...
    def train(self):
        ...
        self.sdf_network.decoder.progress.data.fill_(progress_data)
        ...
        while iter_i in tqdm(range(res_step)):
            ...
            if self.iter_step == 100000:
                self.validate_mesh(world_space=True, resolution=512, threshold=0.0)
