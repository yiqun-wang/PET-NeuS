...
class NeuSRenderer:
    ...
    def up_sample_mid(self, rays_o, rays_d, z_vals, sdf, n_importance, inv_s, gradient):
        """
        Up sampling give a fixed inv_s
        """
        batch_size, n_samples = z_vals.shape
        ...
        cos_val = (rays_d[:, None, :].expand(pts.shape) * gradient.view(pts.shape)).sum(-1)[:,:-1]
    ...
    def cat_z_vals_mid(self, rays_o, rays_d, z_vals, new_z_vals, sdf, last=False, gradient=None):
        batch_size, n_samples = z_vals.shape
        _, n_importance = new_z_vals.shape
        pts = rays_o[:, None, :] + rays_d[:, None, :] * new_z_vals[..., :, None]
        z_vals = torch.cat([z_vals, new_z_vals], dim=-1)
        ...
        if not last:
            new_sdf = self.sdf_network.sdf(pts.reshape(-1, 3)).reshape(batch_size, n_importance)
            with torch.enable_grad():
                gradients = self.sdf_network.gradient(pts.reshape(-1, 3)).squeeze().detach()
            if gradient is not None:
                gradient = torch.cat([gradient.view([sdf.shape[0], sdf.shape[1], 3]), gradients.view(pts.shape)], dim=-2).view(-1, 3)
            sdf = torch.cat([sdf, new_sdf], dim=-1)
            xx = torch.arange(batch_size)[:, None].expand(batch_size, n_samples + n_importance).reshape(-1)
            index = index.reshape(-1)
            sdf = sdf[(xx, index)].reshape(batch_size, n_samples + n_importance)

        return z_vals, sdf, gradient
    ...
    def render_core(self,
        ...
        alpha_ori = alpha
        weights_ori = alpha_ori * torch.cumprod(torch.cat([torch.ones([batch_size, 1]), 1. - alpha_ori + 1e-7], -1), -1)[:, :-1]
        weights_sum_ori = weights_ori.sum(dim=-1, keepdim=True)
